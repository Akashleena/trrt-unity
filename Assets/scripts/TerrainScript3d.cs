...
public class TerrainScript3d : MonoBehaviour {
	...
	void Start () {
		...
		Mapbox.Unity.Map AbstractMap aMap;
		latLon=aMap.UpdateMap();
		var centerLatitudeLongitude = latLon;
			double xDelta = centerLatitudeLongitude.x;
			double zDelta = centerLatitudeLongitude.y;

			terrainSize.x = xDelta > 0 ? Mathd.Min(xDelta, Mapbox.Utils.Constants.LatitudeMax) : Mathd.Max(xDelta, -Mapbox.Utils.Constants.LatitudeMax);
			terrainSize.z = zDelta > 0 ? Mathd.Min(zDelta, Mapbox.Utils.Constants.LongitudeMax) : Mathd.Max(zDelta, -Mapbox.Utils.Constants.LongitudeMax);
			terrainSize.y= 1.0f;
	...
	void TRRTGrow () {
		...
		while(numAttempts < solvingSpeed && nodes.Count < MAX_NUM_NODES) {
			if(needNewTarget) {
               ...
               newnode.l1 = dx/distdc;
               newnode.m1 = dy/distdc;
               newnode.n1 = dz/distdc;
			...
			//calculate the co-ordinates of new node
			 pos = new Vector3(nodes[closestInd].pos.x + stepSize*newnode.l1, nodes[closestInd].pos.y + stepSize*newnode.n1, nodes[closestInd].pos.z + stepSize*newnode.m1);
